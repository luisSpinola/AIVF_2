...
export default function AreaPlot({data, options, globalColors}){
                ...
                <ComposedChart ref={initChart} layout={axesArray[2]} data={data.data} margin={margin}>
                    ...
                    {legend}
                    {
                        data.header.value.map((_, i) => {
                            return <Area key={i} yAxisId="left" stackId={stacked} dataKey={data.header.value[i]} type={options.interpolation} fill={colors[i]} stroke={colors[i]} strokeWidth={options.line_stroke} dot={options.dots} fillOpacity={options.colors_opacity/100} legendType="plainline"> 
                                {labelList}
                            </Area>
                        })
                    }
                ...
                plots.push(
                    <ResponsiveContainer key={data.header.value[i]} width="100%" height={options.height}>
                        <AreaChart layout={axesArray[2]} data={data.data} margin={margin}>
                            ...
                            {grid}
                            {legend}
                            <Area yAxisId="left" dataKey={data.header.value[i]} type={options.interpolation} fill={colors[i]} stroke={colors[i]} strokeWidth={options.line_stroke} dot={options.dots} fillOpacity={options.colors_opacity/100} legendType="plainline">
                                {labelList}
                            </Area>
                        </AreaChart>
                    </ResponsiveContainer>
